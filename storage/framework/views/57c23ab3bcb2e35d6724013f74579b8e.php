...
<?php unset($__defined_vars); ?>
    <label for="toggle" class="flex items-center cursor-pointer">
        <!-- The toggle switch -->
        <div class="relative">
            <input type="checkbox" id="toggle" wire:click="toggleStatus" class="sr-only"
                <?php if($status): echo 'checked'; endif; ?>>
            <div class="block w-14 h-8 rounded-full <?php echo e($status ? 'bg-green-400' : 'bg-gray-400'); ?>" ></div>
            <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform duration-300 ease-in-out <?php echo e($status ? 'transform translate-x-6' : ''); ?>">
            </div>
        </div>
        <!-- Status Text -->
